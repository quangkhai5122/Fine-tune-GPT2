"""Create v19: v17 lr=2e-3 epoch-7 answer-only + v18 strict verifier overlay."""
...
V18_SCRIPT = ROOT / "audit" / "create_v18_candidate_verifier_notebook.py"
NOTEBOOK_PATH = ROOT / "finetune_gpt2_for_math_v19_v17_lr2e3_strict_verifier_overlay.ipynb"

BASE_VERSION = "v18_legal_candidate_verifier_template_retrieval"
NEW_VERSION = "v19_v17_lr2e3_strict_verifier_overlay"


def load_v18_module():
    spec = importlib.util.spec_from_file_location("create_v18_candidate_verifier_notebook", V18_SCRIPT)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Cannot import {V18_SCRIPT}")
...
def patch_source(source: str) -> str:
    ...
    source = source.replace("STAGE_A_LR = 0.001", "STAGE_A_LR = 0.002")
    source = source.replace(
        "train_query_vi_template_retrieval_to_train_response_vi_candidates",
    ...
    source = source.replace(
        ...
        (
            '"notes": "Base SFT is v17-style answer-only epoch 7 at lr=2e-3. '
            "Candidate retrieval uses train query_vi only; candidate answers come from train response_vi "
            'or query-derived numeric/template heuristics.",'
        ),
    )
    return source
